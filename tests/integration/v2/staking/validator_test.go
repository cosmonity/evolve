...
func TestUpdateBondedValidatorsDecreaseCliff(t *testing.T) {
	...
	for i := 0; i < len(validators); i++ {
		...
		val, _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, val, true)
		validators[i] = val
	}
	...
	_, _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, nextCliffVal, true)
...
func TestSlashToZeroPowerRemoved(t *testing.T) {
	...
	validator, _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validator, true)
	assert.DeepEqual(t, valTokens, validator.Tokens)
...
func TestGetValidatorSortingUnmixed(t *testing.T) {
	...
	for i, amt := range amts {
		...
		testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[i], true)
	}
	...
	testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[3], true)
	resValidators, err = f.stakingKeeper.GetBondedValidatorsByPower(f.ctx)
	assert.NilError(t, err)
	...
	testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[4], true)
	resValidators, err = f.stakingKeeper.GetBondedValidatorsByPower(f.ctx)
	assert.NilError(t, err)
	assert.Equal(t, len(resValidators), n)
	assert.Assert(ValEq(t, validators[3], resValidators[0]))
...
func TestGetValidatorsEdgeCases(t *testing.T) {
	...
	for i, power := range powers {
		...
		f.accountKeeper.SetModuleAccount(f.ctx, notBondedPool)
		validators[i], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[i], true)
	}
	...
	validators[0], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[0], true)
	resValidators, err = f.stakingKeeper.GetBondedValidatorsByPower(f.ctx)
	assert.NilError(t, err)
	assert.Equal(t, nMax, uint32(len(resValidators)))
	...
	validators[3], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[3], true)
	resValidators, err = f.stakingKeeper.GetBondedValidatorsByPower(f.ctx)
	assert.NilError(t, err)
	assert.Equal(t, nMax, uint32(len(resValidators)))
	assert.Assert(ValEq(t, validators[0], resValidators[0]))
...
func TestValidatorBondHeight(t *testing.T) {
	...
	validators[1], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[1], true)
	validators[2], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[2], true)
...
func TestApplyAndReturnValidatorSetUpdatesIdentical(t *testing.T) {
	...
	validators[0], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[0], false)
	validators[1], _ = testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[1], false)
...
func TestApplyAndReturnValidatorSetUpdatesMultipleValueChange(t *testing.T) {
	...
	val1, val1Updates := testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[0], true)
	assert.Equal(t, 1, len(val1Updates))
	validators[0] = val1
	updates = append(updates, val1Updates...)

	val2, val2Updates := testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[1], true)
...
func TestApplyAndReturnValidatorSetUpdatesWithCliffValidator(t *testing.T) {
	...
	//  tendermintUpdate set: {} -> {}
	testingUpdateValidatorV2(f.stakingKeeper, f.ctx, validators[2], false)
	applyValidatorSetUpdates(t, f.ctx, f.stakingKeeper, 0)
